...
def acc_to_polydis_pr_mat(acc, length=None):
    length = acc.shape[1] if length is None else length
    pr_mat = np.zeros((length, 128), dtype=np.float32)

    dur = np.zeros(128, dtype=np.float32)

    for t in range(acc.shape[1] - 1, -1, -1):
        is_onset = acc[0, t] > 0
        pr_mat[t, is_onset] = acc[0, t, is_onset] + dur[is_onset]
        if t == 0:
            break
        dur[is_onset] = 0
        dur += acc[1, t]
...
class AccompanimentDataset(HierarchicalDatasetBase):
    ...
    def __init__(self, analyses, shift_high=0, shift_low=0, max_l=128, h=128, n_channels=10,
        ...
        super(AccompanimentDataset, self).__init__(
        ...
        acc_langs = [analysis['languages']['accompaniment'] for analysis in analyses]
        ...
        self.acc_rolls = [acc_lang['acc_roll'] for acc_lang in acc_langs]
    ...
    def get_data_sample(self, song_id, start_id, shift):
        ...
        self.store_acc(song_id, shift)  # note the difference
    ...
    def lang_to_img(self, song_id, start_id, end_id, tgt_lgth=None):
        ...
        acc_roll = self._acc[:, start_id: end_id]
        ...
        img[2: 4, 0: actual_l, 36: 48] = chd_roll[2: 4]
        img[2: 4, 0: actual_l, 24: 36] = chd_roll[4: 6]

        img[4: 6, 0: actual_l, 0: 128] = red_mel_roll
        img[4: 6, 0: actual_l, 36: 48] = red_chd_roll[2: 4]
        img[4: 6, 0: actual_l, 24: 36] = red_chd_roll[4: 6]
    ...
    def get_external_cond(self, start_id):
        acc = self._acc[:, start_id: start_id + self.max_l]
...
def create_accompaniment_datasets(train_analyses, valid_analyses, use_autoreg_cond=True, use_external_cond=False,
                                  multi_phrase_label=False, random_pitch_aug=True, mask_background=True):
    lang_params = LANGUAGE_DATASET_PARAMS['accompaniment']
    autoreg_params = AUTOREG_PARAMS['accompaniment']

    train_dataset = AccompanimentDataset(
    ...
    valid_dataset = AccompanimentDataset(
